...
export default function TinyBalances() {
  ...
  const general_ust_fee = toChainAmount(0.75); //for native balance error
  ...
  const donate = async () => {
    ...
    const donateNativeMsg = donateTinyAmount(
      user_address,
      ANGEL_PROTO_ADDRESS_MAIN,
      balancesObj
    );
    const donateCW20Msgs = donateTinyCW20Amount(
      user_address,
      ANGEL_PROTO_ADDRESS_MAIN,
      tinyCW20s
    );
  ...
  const tinyCW20Setter = async () => {
    ...
    for (const [symbol, info] of Object.entries(cw20Tokens)) {
      ...
      cw20s.push({
        ...
        address: info.token_addr,
      });
    }

    cw20s = cw20s.filter(
      (c) =>
        Number(c.amount) > toChainAmount(lowerlimit) &&
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const swapMap = swaprates.reduce((map: any, obj: any) => {
        map.set(obj.denom, +obj.swaprate);
        return map;
      }, new Map([["uusd", 1]]));
    ...
    <>
      ...
      <SectionWrapper type="balance">
        <TokenContainer
          ...
          array={[...tinyBalances, ...tinyCW20s]}
          swapMap={ustSwapRateMap}
          remove={remove}
        />
        <DescriptionSubmit
          title={
